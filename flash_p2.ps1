...
if ((Test-Path $CACHE_FILE) -and (-not $Force)) {
    try {
        $jsonObj = Get-Content $CACHE_FILE -Raw | ConvertFrom-Json
        $FileCache = @{}
        # Универсальный перевод объекта JSON в Хэш-таблицу для PowerShell 5.1 и выше
        foreach ($prop in $jsonObj.PSObject.Properties) {
            $FileCache[$prop.Name] = $prop.Value
        }
    } catch {
        $FileCache = @{}
    }
...
function Upload-IfChanged{
    param(
        [string]$LocalFile,
        [string]$RemoteFile,
        [string]$SourceFile = ""  # <--- Добавляем параметр для отслеживания оригинала
    )

    # Если передан исходный файл, считаем хэш от него, иначе от текущего
    $fileToHash = if (![string]::IsNullOrWhiteSpace($SourceFile)) { $SourceFile } else { $LocalFile }
    $hash = Get-Hash $fileToHash
    ...
    Get-ChildItem $pattern -ErrorAction SilentlyContinue | ForEach-Object {
        ...
        if ($USE_MPY -and $_f.Name -ne "main.py") {
            ...
            if (($LASTEXITCODE -eq 0) -and (Test-Path $_mpy)) {
                ...
                # Модифицированная строка: передаем исходный .py файл для сверки кэша
                Upload-IfChanged -LocalFile $_mpy -RemoteFile $remote -SourceFile $_f.FullName
